...
#ifdef QT_GUI
const char *walletUrl = "https://www.oscoin.info/";
const char *walletDownloadsUrl = "https://www.oscoin.info/downloads/";
const char *forumsUrl = "http://www.oscoinforums.com";
bool fRestart = false;
bool fRescan = false;
bool fEncrypt = false;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OSCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OSCoin
    // Mac: ~/Library/Application Support/OSCoin
    // Unix: ~/.oscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OSCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "oscoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong OSCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("OSCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
