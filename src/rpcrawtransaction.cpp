...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid OSCoin address: ")+input.get_str());
...
Value createrawtext(const Array& params, bool fHelp)
{
    ...
    if (message.size() > 100000)
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Your Message is longer then 100 kb");
