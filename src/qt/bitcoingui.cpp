    ...
    rpcConsole(0)
{
    ...
    setWindowTitle(tr("OSCoin Wallet"));
    ...
    progressBar->setMinimumWidth(450);
...
void BitcoinGUI::lockWalletFeatures(bool lock)
{
    ...
    else
    {
        ...
        QSettings settings("OSCoin", "OSCoin-Qt");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send OSCoin"));
    ...
    aboutAction = new QAction(QIcon(":/icons/about"), tr("&About OSCoin"), this);
    aboutAction->setToolTip(tr("Show information about OSCoin"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for OSCoin"));
    ...
    forumAction = new QAction(QIcon(":/icons/bitcoin"), tr("OSCoin &Forums"), this);
    forumAction->setToolTip(tr("Go to the OSCoin forums."));
    webAction = new QAction(QIcon(":/icons/site"), tr("www.oscoin.info"), this);
    webAction->setToolTip(tr("Go to OSCoin website."));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("OSCoin Wallet") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("OSCoin Wallet"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%1 active connection%2 to the OSCoin network").arg(count).arg(count == 1 ? "" : "s"));
}
    ...
    QString strMessage =
        tr("To send this transaction it requires a fee of %1, "
          ...
          "Do you want to continue?").arg(
                BitcoinUnits::formatWithUnitFee(BitcoinUnits::OSC, nFeeRequired));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid OSCoin address or malformed URI parameters."));
    }
    ...
    if (walletModel->getEncryptionStatus() == WalletModel::Unlocked)
    {
        ...
        if (!address.SetString(strAddress))
        {
            QMessageBox::warning(this, tr("Export Private Key"),
                tr("This is an invalid OSCoin address"),
...
void BitcoinGUI::checkForUpdate()
{
    ...
    if (fNewVersion)
    {
        ...
        std::string basename = GetArg("-vFileName","oscoin-qt");
