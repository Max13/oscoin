...
void setupAmountWidget(QLineEdit *widget, QWidget *parent)
{
    ...
    amountValidator->setDecimals(BitcoinUnits::decimals(BitcoinUnits::OSC));
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::OSC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert oscoin:// to oscoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("oscoin://"))
    {
        uri.replace(0, 12, "oscoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OSCoin.lnk";
}
...
{
    return GetAutostartDir() / "oscoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("OSCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  oscoin-qt [" + tr("command-line options") + "]                     " + "\n";
