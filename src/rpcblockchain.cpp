...
Value bootstrap(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "bootstrap [overwrite_oscoin_conf=false]\n"
            "Download blockchain and optionally a current oscoin.conf from www.oscoin.info.\n"
    ...
    int err = DownloadFile("https://www.oscoin.info/downloads/bootstrap.zip", pathBootstrapZip);
    if (err != 0)
    {
    ...
    result.push_back(Pair("comment", "Bootstrap successful; oscoind has been stopped, please restart."));
