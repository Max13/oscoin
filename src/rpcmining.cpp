...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("stakeinterest",    GetCurrentInterestRate(pindexBest).convert_to<double>()));
    obj.push_back(Pair("stakeinflation",    GetCurrentInflationRate(averageStakeWeight).convert_to<double>()));
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "OSCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "OSCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "OSCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "OSCoin is downloading blocks...");
